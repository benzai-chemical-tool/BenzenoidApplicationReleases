...
public class StatCoverWindowOutput extends SolutionWindowOutput {
	private StatistiqueStructure stats;
	public StatCoverWindowOutput(FlowPane pane, StatistiqueStructure stats) {
		super(pane);
		this.stats = stats;
	}
	...
	public void output(BenzenoidSolution solution) {
		...
		// Courbe comparaison Clar/Kekule
		Color squareColor = stats.kekuleMatchesClar() ? Color.BLACK : Color.RED;
		addSquareToDrawing(getDrawing(), 0, 0, size, squareColor);
		for(int hex = 0 ; hex < dimension * dimension ; hex ++) {
			float y = (hex / (int)dimension);
			float x = (float) (hex % dimension + (nbCouronnes - y / 2.));
			//System.out.println(hex + ":" + x + "," + y + "=" + stats.getCompteursCycles()[hex]);
			int pourcentCycle = stats.getNbKekuleCovers() > 0 ? stats.getCompteursCycles()[hex]  * 100 / stats.getNbKekuleCovers() : 0;
			int pourcentRond = stats.getNbClarCovers() > 0 ? stats.getCompteursRonds()[hex]  * 100 / stats.getNbClarCovers() : 0;
			if(stats.getCompteursCycles()[hex] > 0)
				addCycleTextToDrawing(getDrawing(), x * size, (float) (y * size * Math.sqrt(3.)/2.), size / 3, "" + pourcentCycle);
			if(stats.getCompteursRonds()[hex] > 0)
				addRondTextToDrawing(getDrawing(), x * size, (float) (y * size * Math.sqrt(3.)/2.), size / 3, ""+ pourcentRond);
			if(stats.getCompteursCycles()[hex] > 0 || stats.getCompteursRonds()[hex] > 0)
				addCircleToDrawing(getDrawing(), pourcentCycle , size - pourcentRond, 5);
		}
		...
		for(int hex = 0 ; hex < dimension * dimension ; hex ++)
			for(int j = 0; j < 6; j++) {
				...
				if(stats.getCompteursLiaisons()[hex][j] > 0)
					addBondTextToDrawing(getDrawing(), x * size, (float) (y * size * Math.sqrt(3.)/2.), size, j, ""+ stats.getCompteursLiaisons()[hex][j]  * 100 / stats.getNbKekuleCovers());
				if(stats.getCompteursCelibataires()[hex][j] > 0)
					addCelibataireTextToDrawing(getDrawing(), x * size, (float) (y * size * Math.sqrt(3.)/2.), size, j, ""+ stats.getCompteursCelibataires()[hex][j]  * 100 / stats.getNbKekuleCovers());
		}
		...
		Text nbClar = new Text(0, size, "" + stats.getNbClarCovers());
		nbClar.setFont(new Font(18));
		nbClar.setStroke(Color.BLUEVIOLET);
		getDrawing().getChildren().add(nbClar);
	...
	private void addBondTextToDrawing(Group drawing, float xcenter, float ycenter, float size, int side, String texte) {
		...
		Text text; 
		switch(side) {
			case 0 : text = new Text(xcenter + c * 0.5, ycenter - r * 0.75, texte); break;
			case 1 : text = new Text(xcenter + c, ycenter, texte); break;
			case 2 : text = new Text(xcenter + c * 0.5, ycenter + r * 0.75, texte); break;
			case 3 : text = new Text(xcenter - c * 0.5, ycenter + r * 0.75, texte); break;
			case 4 : text = new Text(xcenter - c, ycenter, texte); break;
			case 5 : text = new Text(xcenter - c * 0.5, ycenter - r * 0.75, texte); break;
			default : text = null;
		}
	...
	private void addCelibataireTextToDrawing(Group drawing, float xcenter, float ycenter, float size, int carbonePosition, String texte) {
		float r = (float) (size / Math.sqrt(3));
		float c = r * (float) (Math.sqrt((double)3)/2);
		Text text;  
		switch(carbonePosition) {
			case 0 : text = new Text(xcenter, ycenter - r , texte); break;
			case 1 : text = new Text(xcenter + c, ycenter - r / 2, texte); break;
			case 2 : text = new Text(xcenter + c, ycenter + r / 2, texte); break;
			case 3 : text = new Text(xcenter, ycenter + r, texte); break;
			case 4 : text = new Text(xcenter - c, ycenter + r / 2, texte); break;
			case 5 : text = new Text(xcenter - c, ycenter - r / 2, texte); break;
			default : text = null;
		}
