...
public class OldLexLead {
	    ...
	    for(j = 0 ; j < largeur; j++)
	        for(i = 0 ; i < largeur ; i++)
	        	if(inCoronenoid(i, j, generalModel) && inCoronenoid(p.from(xy2i(i,j, generalModel)), generalModel)) {
	        		//System.out.println("-" + y.getName() + " " + xy2i(i,j) + " -" + p.from(xy2i(i,j)));
	        		model.addClauses(LogOp.or(LogOp.nor(y), benzenoidVertices[xy2i(i,j, generalModel)], LogOp.nor(benzenoidVertices[p.from(xy2i(i,j, generalModel))])));
	        		if(j != largeur - 1 || i != largeur - 1){
	        			...
	        			//System.out.println((y.getName()+1) + " -" + y.getName() + " -" + p.from(xy2i(i,j)));
	        			model.addClauses(LogOp.or(yp1, LogOp.nor(y), LogOp.nor(benzenoidVertices[p.from(xy2i(i,j, generalModel))])));
	        		}
	        		y = yp1;
	        		yp1 = model.boolVar();
	        	}
	...
	/***
	 * Poste les 11 contraintes d'élimination de symétries : 6 rotations avec ou sans symétrie diagonale
	 */
